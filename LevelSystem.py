...
async def level_up(users, user, message):
  ...
  if lvl_start < lvl_end:
    await message.channel.send(f':tada: GG! {user.mention} hat nun Level {lvl_end} erreicht :tada:')
...
async def level(ctx, member: discord.Member = None):
  if member == None:
    ...
    await ctx.send(f'{ctx.author.mention} Du bist auf Level {userlvl}!')
  else:
    userlvl2 = users[f'{member.id}']['level']
    await ctx.send(f'{member.mention} ist auf Level {userlvl2}!')